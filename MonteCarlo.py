...
The simulation engine lives in montecarlo/engine.py so the same code powers
both this script and the deployed API. Edit the assumptions at the bottom and
run `python MonteCarlo.py` to print a summary and save a chart to results/.
"""
...
def plot_results(engine: MonteCarloEngine, save_path: str = None):
    ...
    ax1.set_title(f"{a.ticker} — Terminal Price Distribution ({a.horizon_years:g}-Year, {a.n_simulations:,} sims)")
    ...
    ax4 = axes[1, 1]
    impacts = engine.tornado()
    y_pos = np.arange(len(impacts))
    for i, imp in enumerate(impacts):
        low, high = imp["low"], imp["high"]
    ...
    ax4.set_yticklabels([x["name"] for x in impacts], color="#c9d1d9")
...
def print_summary(engine: MonteCarloEngine):
    ...
    print(f"  Simulations: {a.n_simulations:,}  |  Horizon: {a.horizon_years:g} Years")
    ...
    print(f"  {'Expected Return':<32} {probs['expected_return']:>14.1%}")
...
if __name__ == "__main__":
    ...
    plot_results(engine)  # saves as results/MYRG_monte_carlo.png
